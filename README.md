...
class Fahrzeug : virtual public mobs::ObjectBase
{
   public:
     ...
     MemVar(std::string, fahrzeugTyp);      // Makro für eine Member-Variable
...
* MemVar für Basistypen
* MemObj für Objekte
* MemVarVector für Vektor von Basistypen
...
~~~~~~~~~~
Die Vektoren werden bei Zugriffen automatisch vergrößert, sodass eine Überprüfung entfallen kann.
...
Dazu existiert eine Methode, um beliebige Mobs-Objekte einer Basisklasse in einen Vektor<MobsUnion> aufzunehmen. Das Objekt eines solchen 
MobsUnions kann zur Laufzeit ausgetauscht werden.

### Null-Values
Alle Elemente haben zusätzliche Eigenschaften für NULL.
Wurde eine Variable mit "USENULL" definiert, so wird sie standardmäßig mit null initialisiert.
...
* useCompactValues() Werte in Kurzform akzeptieren (z.B. Zahl anstatt enum-Text)
* useExtendedValues() Werte in Langform akzeptieren (z.B. enum-Text, Datum)
...
### DatabaseInterface
Das DatabaseInterface dient als Schnittstelle für alle Datenbankoperationen. Dazu muss in den Objektdefinitionen
...
Soll die Ausgabe sortiert erfolgen, so kann bei query() und qbe() eine Sortierung angegeben werden:
...
Um verschachtelte Objekte in eine SQL-Datenbank schreiben zu können stehen folgende Feature-Token
zur Verfügung:
...
als der finale Commit für alle Datenbanken am Ende der Transaktion nacheinander abläuft.
 
### Objekt-Versionierung
Wird über das Token VERSIONFIELD eine Versions-Variable definiert, wird dort automatisch die
Version des Datensatzes verwaltet. Je Objekt darf nur eine Versions-Variable existieren. Sie muss 
...
Bei dem SQL-Feature VERSIOBFIELD mit automatischen Master-Detail-Tabellen, wird die Version nur in der 
Mater-Tabelle verwaltet.

### Audit Trail
Für jedes Objekt kann definiert werden, ob eine automatische Audit-Trail-Überwachung aktiviert
werden soll.
...
Zusätzlich werden Zeit, User (Id) und Job-Id mitprotokolliert. Über die Job-Id kann dann bei jedem Programmstart auch eine 
weitere Information bezüglich jedes ausgeführten Programms erfasst werden.
...
mit den streambuffer-Erweiterungen
* CryptIstrBuf für lesende streams
* CryptOstrBuf für schreibende streams
...
Zusätzlich existiert eine tcpstream Klasse, um eine Kommunikation über TCP-Sockets zu ermöglichen.
~~~~~~~~~~cpp
  mobs::tcpstream con("foo.org", "8080");
  if (not con.is_open())
...
* tcpstream.h iostream-Klasse für TCP-Verbindungen
