...
class MrpcClient : public mobs::Mrpc {
public:
  MrpcClient(std::istream &iStr, std::ostream &oStr) :
          Mrpc(iStr, oStr, &mrpcSession, false) {}
...
public:
  ...
  std::string loginReceived(const std::vector<u_char> &cipher, std::string &info) override {
    std::string keyId = receiveLogin(cipher, *session, privKey, "");
    info = "welcome";
...
TEST(mrpcTest, MrpcClient) {
  ...
  MrpcClient client(strIn, strOut);
  for (;;) {
...
TEST(mrpcTest, MrpcClientServer) {
  ...
  MrpcClient client(strOut, strIn);
